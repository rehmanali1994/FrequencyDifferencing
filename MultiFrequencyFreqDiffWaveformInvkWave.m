...
fDIFF = (0.15:0.05:0.95)*(1e6); % Synthesize these Frequencies [Hz]
fDATA = (0.6:0.05:5.0)*(1e6); % Extract these Frequencies from Data [Hz]
...
for fDIFFidx = 1:numel(fDIFF)
    ...
    REC_DATA_FD(:,:,fDIFFidx) = sum(REC_DATA2.*conj(REC_DATA),3) ./ ...
        sqrt(sum(REC_DATA.*conj(REC_DATA),3));
end

% Extract the Desired Frequency for Standard Waveform Inversion
fDATA_SoS = (0.6:0.05:1.25)*(1e6); % Frequencies for SoS-only Iterations [Hz]
fDATA_SoSAtten = (0.625:0.05:1.275)*(1e6); % Frequencies for SoS/Attenuation Iterations [Hz]
...
% Ground-Truth Sound Speed Image
VEL_TRUE = interp2(xi_orig, yi_orig, C, Xi, Yi);
...
attenrange = 2*[-1,1]; % For reconstruction display [dB/(cm MHz)]
...
for f_idx = 1:numel(fDATA)
    ...
    for iter_f_idx = 1:(niterSoSPerFreq(f_idx)+niterAttenPerFreq(f_idx))
        ...
        xlabel('Lateral [m]'); ylabel('Axial [m]'); colorbar; colormap gray;
        subplot(2,3,2); imagesc(xi_orig,yi_orig,VEL_TRUE,crange);
...
title(['Estimated Attenuation ', num2str(iter)]); axis image;
xlabel('Lateral [m]'); ylabel('Axial [m]'); colorbar; colormap gray;
subplot(2,2,3); imagesc(xi,yi,VEL_TRUE,crange);
title('True Wave Velocity'); axis image;
xlabel('Lateral [m]'); ylabel('Axial [m]'); colorbar; colormap gray;
...
% RMS Error in Sound Speed Reconstruction
Ri = sqrt(Xi.^2 + Yi.^2); max_radius = 0.09;
RMSE = std(VEL_ESTIM(Ri<max_radius)-VEL_TRUE(Ri<max_radius));
