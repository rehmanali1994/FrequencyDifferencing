...
filename = 'VSX512_AlcoholBrain_6_14_24_Axial'; % 'VSX512_AlcoholBrain_6_14_24_Sagittal'
...
% Rotate Ring
theta = -110; % angle to rotate ring (in degrees)
R = [cosd(theta) -sind(theta); sind(theta) cosd(theta)];
transducerPositionsXY = R*transducerPositionsXY;

% Geometric TOF Based on Sound Speed
c_geom = 1600; % Sound Speed [mm/us] 1590
...
sos_perc_change_pre = 0.03; sos_perc_change_post = 0.03; %Inf; 
...
for tx_element = 1:numElements
    ...
    fsa_rf_data(:,:,tx_element) = window.*fsa_rf_data(:,:,tx_element);
end

%% Frequency Differencing

% Waveform Inversion Based on Frequency Differences
fDIFF = (0.1:0.05:0.55)*(1e6); % Synthesize these Frequencies [Hz]
fDATA = (1.0:0.05:5.0)*(1e6); % Extract these Frequencies from Data [Hz]
niterPerFreqFD = 2*ones(size(fDIFF)); % Number of Iterations Per Frequency 
    % Avoid too many iteration - Dont overfit to frequency-differenced data
% Discrete-Time Fourier Transform (DTFT) - Not an FFT though!
DTFT = exp(-1i*2*pi*fDATA'*time)*mean(diff(time)); 
REC_DATA = permute(pagemtimes(DTFT, fsa_rf_data), [3,2,1]);
% Synthesize Frequency Differences
REC_DATA_FD = zeros(numElements,numElements,numel(fDIFF));
for fDIFFidx = 1:numel(fDIFF)
    DTFT2 = exp(-1i*2*pi*(fDATA+fDIFF(fDIFFidx))'*time) * mean(diff(time)); 
    REC_DATA2 = permute(pagemtimes(DTFT2, fsa_rf_data), [3,2,1]);
    REC_DATA_FD(:,:,fDIFFidx) = sum(REC_DATA2.*conj(REC_DATA),3) ./ ...
        (sum(REC_DATA.*conj(REC_DATA),3));
end

% Extract the Desired Frequency for Standard Waveform Inversion
fDATA_SoS = (0.45:0.05:1.0)*(1e6); % Frequencies for SoS-only Iterations [Hz]
fDATA_SoSAtten = (0.475:0.05:1.025)*(1e6); % Frequencies for SoS/Attenuation Iterations [Hz]
...
% Overwrite Certain Variables for Use in Waveform Inversion
fDATA = [fDIFF, fDATA];
niterSoSPerFreq = [niterPerFreqFD, niterSoSPerFreq];
niterAttenPerFreq = [0*niterPerFreqFD, niterAttenPerFreq];
REC_DATA = cat(3, REC_DATA_FD, REC_DATA);
clearvars twin window fsa_rf_data time times_tx TIME TIMES_TX DTFT DTFT2;

%% Computational Grid and Preprocessing

% Create Sound Speed Map and Transducer Ring
dxi = 0.4e-3; xmax = 120e-3;
...
c_init = 1600; % Initial Homogeneous Sound Speed [m/s] Guess
...
crange = [1600, 1680]; % For reconstruction display [m/s]
